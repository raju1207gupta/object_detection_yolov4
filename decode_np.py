...
class Decode(object):
    ...
    def detect_image(self, image, draw_image=True):
    ...
    def detect_video(self, video_path):
        ...
        vout = cv2.VideoWriter(f"{str(os.path.dirname(video_path))}/out_{(os.path.basename(video_path)).split('.')[0]}.avi", fourcc, 2, (1280,720))
        ##vout.open(f"{str(os.path.dirname(video_path))}/out_{str(os.path.basename(video_path))}", fourcc, 10, (1280,720), True)
        ...
        while True:
            ...
            sec = (currentTime-fromTime).seconds
            if sec>=2:
                fromTime = currentTime
                image, boxes, scores, classes = self.detect_image(frame)
